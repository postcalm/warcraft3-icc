...
function MeleeTownHall(townid, unitid)
	if TownHasMine(townid) and -TownHasHall(townid) then
...
function CommonSleepUntilTargetDead(target, reform)
	while true do
		...
		if UnitInvis(target) and -IsUnitDetected(target, ai_player) then break end
...
function SingleMeleeAttack(needs_exp, has_siege, major_ok, air_units)
	...
	if hall ~= nil and (can_siege or -IsTowered(hall)) then
