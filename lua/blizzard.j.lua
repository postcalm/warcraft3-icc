...
function MeleeGetAllyCount(whichPlayer)
	...
	while true do
		indexPlayer = Player(playerIndex)
		if PlayersAreCoAllied(whichPlayer, indexPlayer) and -bj_meleeDefeated[playerIndex] and (whichPlayer ~= indexPlayer) then
...
function MeleeCheckForLosersAndVictors()
	...
	while true do
		...
		if ( not bj_meleeDefeated[playerIndex] and -bj_meleeVictoried[playerIndex]) then
...
function MeleeCheckForCrippledPlayers()
	...
	while true do
		...
		elseif (bj_playerIsCrippled[playerIndex] and -isNowCrippled) then
...
function SetDNCSoundsDay()
	...
	if (ToD >= bj_TOD_DAWN and ToD < bj_TOD_DUSK) and -bj_dncIsDaytime then
